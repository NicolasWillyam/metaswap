...
import { SwapInfo } from "@/features/swap/components/swap/SwapInfo";
...
const SwapPage = () => {
  ...
  useEffect(() => {
    ...
    const fromToken = tokens.find((t: any) => t.symbol === fromSymbol);
    const toToken = tokens.find((t: any) => t.symbol === toSymbol);

    if (fromToken) dispatch(setSelectedToken(fromToken));
    if (toToken) dispatch(setSwappedToken(toToken));
    if (amountParam && !isNaN(Number(amountParam))) {
      dispatch(setFromValue(Number(amountParam)));
    }
  ...
  return (
    <div className="py-20">
      <div className="w-full sm:min-w-full flex flex-col justify-center">
